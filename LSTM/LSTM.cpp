...
double LSTM::forward(const vector<vector<double>>& X, vector<double>& predictions) {
    ...
    for (int i = seq_length; i < n; ++i) {
        vector<vector<double>> h_t(seq_length + 1, vector<double>(hidden_size, 0.0));
        vector<vector<double>> C_t(seq_length + 1, vector<double>(hidden_size, 0.0));

        for (int t = 0; t < seq_length; ++t) {
            vector<double> x_t(input_size);
            for (int j = 0; j < input_size; ++j) {
                x_t[j] = X[i - seq_length + t][j];
            }
            ...
            vector<double> f_t = sigmoid(vector_add(matrix_vector_mult(Wf, concat_t), bf));
            vector<double> i_t = sigmoid(vector_add(matrix_vector_mult(Wi, concat_t), bi));
            vector<double> c_tilde = tanh(vector_add(matrix_vector_mult(Wc, concat_t), bc));
            vector<double> o_t = sigmoid(vector_add(matrix_vector_mult(Wo, concat_t), bo));
            ...
            if (i == seq_length && t == seq_length - 1) {
                double f_mean = accumulate(f_t.begin(), f_t.end(), 0.0) / f_t.size();
                double i_mean = accumulate(i_t.begin(), i_t.end(), 0.0) / i_t.size();
...
void LSTM::train(const vector<vector<double>>& X, int epochs) {
    ...
    X_all = X;
    X_train = vector<vector<double>>(X_all.begin(), X_all.begin() + train_size);
    X_test = vector<vector<double>>(X_all.begin() + train_size, X_all.end());

    double best_test_mse = numeric_limits<double>::max();
    int patience = 20;
    int wait = 0;
    ...
    for (int e = 0; e < epochs; ++e) {
        ...
        for (int i = seq_length; i < train_size; ++i) {
            vector<vector<double>> h_t(seq_length + 1, vector<double>(hidden_size, 0.0));
            vector<vector<double>> C_t(seq_length + 1, vector<double>(hidden_size, 0.0));
            vector<vector<double>> f_t(seq_length, vector<double>(hidden_size));
            vector<vector<double>> i_t(seq_length, vector<double>(hidden_size));
            vector<vector<double>> c_tilde(seq_length, vector<double>(hidden_size));
            vector<vector<double>> o_t(seq_length, vector<double>(hidden_size));
            vector<vector<double>> concat(seq_length, vector<double>(input_size + hidden_size));
            ...
            double dpred = 2.0 * (pred - X_train[i][0]) / (train_size - seq_length);

            vector<double> dh_next(hidden_size, 0.0);
            vector<double> dC_next(hidden_size, 0.0);
            for (int j = 0; j < hidden_size; ++j) {
                dh_next[j] = dpred * Wd[j];
                dWd[j] += dpred * h_t[seq_length][j];
            }
            dbd += dpred;
            ...
            for (int t = seq_length - 1; t >= 0; --t) {
                ...
                for (int j = 0; j < hidden_size; ++j) {
                    dbf[j] += max(min(df_t[j], clip_value), -clip_value);
                    dbi[j] += max(min(di_t[j], clip_value), -clip_value);
                    dbc[j] += max(min(dc_tilde[j], clip_value), -clip_value);
                    dbo[j] += max(min(do_t[j], clip_value), -clip_value);
                ...
                for (int j = 0; j < hidden_size; ++j) {
                    ...
                    dh_next[j] = max(min(dh_next[j], clip_value), -clip_value);
                }
                dC_next = elementwise_mult(dC_t, f_t[t]);
                for (int j = 0; j < hidden_size; ++j) {
                    dC_next[j] = max(min(dC_next[j], clip_value), -clip_value);
                }
            }
        }
        ...
        if (test_mse < best_test_mse) {
            ...
            wait = 0;
            save_model("lstm_model_best.json");
        } else {
            wait++;
            if (wait >= patience) {
...
double LSTM::predict(const vector<vector<double>>& X) {
    ...
    vector<double> h_t(hidden_size, 0.0);
    vector<double> C_t(hidden_size, 0.0);
    ...
    for (const auto& x : X) {
        ...
        vector<double> f_t = sigmoid(vector_add(matrix_vector_mult(Wf, concat), bf));
        vector<double> i_t = sigmoid(vector_add(matrix_vector_mult(Wi, concat), bi));
        vector<double> c_tilde = tanh(vector_add(matrix_vector_mult(Wc, concat), bc));
        vector<double> o_t = sigmoid(vector_add(matrix_vector_mult(Wo, concat), bo));
